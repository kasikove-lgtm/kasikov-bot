...
async def adm_open_day(cb: types.CallbackQuery):
    if not is_admin(cb.from_user.username): return
    set_state(cb.from_user.id, {"step": "adm_open_day_date"})
    await cb.message.answer("Дата для открытия всех слотов *ГГГГ-ММ-ДД*:", parse_mode="Markdown")

@dp.callback_query(F.data == "adm_open_week")
async def adm_open_week(cb: types.CallbackQuery):
    if not is_admin(cb.from_user.username): return
    set_state(cb.from_user.id, {"step": "adm_week_start"})
    await cb.message.answer("Дата *начала* недели *ГГГГ-ММ-ДД*:", parse_mode="Markdown")
...
async def adm_quick_week(cb: types.CallbackQuery):
    ...
    db_set("slots", slots)
    await cb.answer()
    await cb.message.answer(f"✅ Открыта рабочая неделя пн-пт с {monday.strftime('%d.%m')}",
...
async def adm_copy_week(cb: types.CallbackQuery):
    ...
    for i in range(7):
        ...
        if last_ds in slots: slots[this_ds] = slots[last_ds].copy(); count += 1
    db_set("slots", slots)
    await cb.answer()
    await cb.message.answer(f"✅ Скопировано {count} дней с прошлой недели", reply_markup=admin_menu())

@dp.callback_query(F.data == "adm_block_month")
async def adm_block_month(cb: types.CallbackQuery):
    if not is_admin(cb.from_user.username): return
    set_state(cb.from_user.id, {"step": "adm_block_month_input"})
    await cb.message.answer("Введи месяц *ГГГГ-ММ* (например `2026-08`):", parse_mode="Markdown")

@dp.callback_query(F.data == "adm_manual_book")
async def adm_manual_book(cb: types.CallbackQuery):
    if not is_admin(cb.from_user.username): return
    set_state(cb.from_user.id, {"step": "adm_manual_date"})
    now = date.today()
    await cb.message.answer("Выбери дату для записи клиента:",
                             reply_markup=admin_calendar(now.year, now.month))
...
async def handle_text(msg: types.Message):
    ...
    if state.get("step") == "adm_manual_tg":
        ...
        if not state.get("adm_time"):
            state["step"] = "adm_manual_time"
            set_state(uid, state)
            await msg.answer("Введи время записи (`14:00`):", parse_mode="Markdown")
        else:
    ...
    if state.get("step") == "adm_week_start":
        if not re.match(r"^\d{4}-\d{2}-\d{2}$", text):
            await msg.answer("❌ Формат: `2026-05-22`", parse_mode="Markdown")
            return
        state["week_start"] = text; state["step"] = "adm_week_end"
        set_state(uid, state)
    ...
    if state.get("step") == "adm_week_end":
        ...
        cur = start; count = 0
        while cur <= end:
            slots[cur.strftime("%Y-%m-%d")] = ALL_DAY_SLOTS.copy()
            count += 1; cur += timedelta(days=1)
        db_set("slots", slots)
        clear_state(uid)
        await msg.answer(f"✅ Открыто *{count} дней*", parse_mode="Markdown",
                         reply_markup=admin_menu())
        return
