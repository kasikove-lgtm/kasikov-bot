"""
Бот Евгения Касикова v6.0 — финальная версия
"""
...
def db_init():
    for k,v in [("slots",{}),("appts",{}),("admin_chats",[]),
                ("blocked",[]),("logs",[]),("blocked_dates",[]),
                ("users",[]),("violations",{}),("regulars",[]),
                ("states",{}),("drip",[]),("invited",[]),
...
# ── STATE (shelve — защита от перезапуска) ──────────────────────────────
def get_st(uid):
...
BAD = ["блять","бля","блядь","хуй","хуйня","хуёво","хуево","хуёвый","хуевый",
       "хуила","хуило","пиздец","пизда","пиздатый","пиздануть","пиздить",
...
T_FEEDBACK = """Независимо от того, будете ли вы работать со мной дальше или нет - хочу попросить короткую обратную связь по нашей первой сессии. Это помогает мне лучше понимать, что в работе было полезно.
...
Если вы не против, я могу использовать ваш ответ в обезличенном виде - без имён и любых деталей по которым можно вас узнать - как обобщённую обратную связь о работе.
...
def kb_back(): return InlineKeyboardMarkup(inline_keyboard=[
...
def cal_admin(year, month):
    ...
    rows.append([
        InlineKeyboardButton(text="✏️ Записать",            callback_data="adm_book_new"),
...
async def bg_loop():
    """Напоминания, воронка, follow-up, проверка сессий"""
    while True:
        ...
        try:
            ...
            for ds, recs in appts.items():
                for rec in recs:
                    ...
                    # ── Напоминание если клиент не подтвердил через 30 мин ──
                    ...
                    try:
                        ...
                        end_dt = appt_dt + timedelta(minutes=dur_m+5)
                        past_end = (now - end_dt).total_seconds()/60
                    except: past_end = -1
 
                    if 0 <= past_end <= 2 and not rec.get("session_asked"):
                        tl = "💼 Платная" if rec.get("type")=="paid" else "🆓 Бесплатная"
            ...
            # ── Follow-up: напоминание Евгению через 24ч ──────────────
...
async def cmd_handler(msg: types.Message):
    ...
    # КРИТИЧНО: команды всегда сбрасывают состояние
    clr_st(uid)
    reg_user(uid)
    log_act(uid, msg.from_user.username, msg.text)
 
    if is_admin(msg.from_user.username):
        chats = db_get("admin_chats",[])
        if uid not in chats: chats.append(uid); db_set("admin_chats",chats)
 
    if msg.text and msg.text.startswith("/stop"):
        if is_admin(msg.from_user.username):
...
async def slot_taken(cb: types.CallbackQuery):
...
async def how_work(cb: types.CallbackQuery):
    ...
    await asyncio.sleep(0.3)
    await cb.message.answer(T_HOW3, parse_mode="Markdown",
...
async def check_sub(cb: types.CallbackQuery):
    if await is_sub(cb.from_user.id):
        ...
        await cb.message.answer(
            f"✅ Держите гайд!\n\n👉 {LEAD}\n\n"
            "Если захочется разобрать свою ситуацию лично - "
...
async def uslot_sel(cb: types.CallbackQuery):
    ...
    if ctype in ("free","reschedule"):
        ...
        await cb.message.answer(
            ...
            f"по желанию опишите пожалуйста что сейчас у вас происходит:",)
    else:
...
async def plat_sel(cb: types.CallbackQuery):
    ...
    # Ручная запись — платформа
    if st.get("step") == "adm_manual_plat":
        st["platform"] = plat; st["step"] = "adm_manual_type"
        set_st(uid, st)
        await cb.answer()
        kb = InlineKeyboardMarkup(inline_keyboard=[
    ...
    ck   = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Подтвердить",
                              callback_data=f"confirm_{ds}_{tm}")],
        [InlineKeyboardButton(text="↩️ Назад", callback_data="main")],
    ])
    await cb.message.answer(
        f"📋 *Проверьте данные:*\n\n"
...
async def confirm_book(cb: types.CallbackQuery):
    ...
    if any(r["user_id"]==uid and r["time"]==tm for r in appts[ds]):
        await cb.answer("Вы уже записаны на это время", show_alert=True)
        return
    rec = {"user_id":uid,"name":name,"time":tm,"username":uname,
    ...
    appts[ds].append(rec); db_set("appts",appts)
    clr_st(uid)
    await cb.message.delete()
    tl = "💼 Платная" if ctype=="paid" else "🆓 Бесплатная"
    ck = InlineKeyboardMarkup(inline_keyboard=[
    ...
    await cb.message.answer(
        f"✅ Запись принята!\n\n"
        f"📅 {ds} в {tm} МСК\n{tl} | ⏱ {dur}\n📱 {plat}\n\n"
        f"Евгений скоро подтвердит и пришлёт ссылку на видеочат.",
        reply_markup=ck)
...
async def my_appts(cb: types.CallbackQuery):
    ...
    rows.append([InlineKeyboardButton(text="↩️ Назад", callback_data="main")])
    if len(rows)==1:
        await cb.message.answer("У вас нет предстоящих записей.",
                                 reply_markup=kb_back())
    else:
...
async def cli_ok(cb: types.CallbackQuery):
    ...
    db_set("appts",appts)
    await cb.answer("✅ Отлично! Ждём вас!", show_alert=False)
...
async def cli_cancel(cb: types.CallbackQuery):
    ...
    if ds in appts:
        ...
        db_set("appts",appts)
    await cb.answer()
    await cb.message.answer("Запись отменена. Если захотите перезаписаться - "
                             "напишите: @kasikovevgenii", reply_markup=kb_main())
...
async def cli_move(cb: types.CallbackQuery):
    ...
    await cb.message.answer("Выберите новую дату:",
        reply_markup=cal_user(today.year, today.month))
    await notify_adm(f"🔄 Клиент хочет перенести запись\n🆔 @{cb.from_user.username or 'нет'}")
 
# ════════════════════════════════════════════════════════════
#  КОЛБЭКИ ЕВГЕНИЯ (подтверждение, отмена, перенос)
# ════════════════════════════════════════════════════════════
...
async def adm_ok(cb: types.CallbackQuery):
    ...
    try:
        ...
        await bot.send_message(uid,
            ...
            f"📅 {ds} в {tm} МСК\n📱 {plat}\n"
            f"{'🔗 '+link if link else ''}\n\n"
            f"За час до сессии вам придёт напоминание.\n"
            f"Просьба подтвердить участие.",
            reply_markup=ck)
    except: pass
...
async def adm_cncl(cb: types.CallbackQuery):
    ...
    try:
        await bot.send_message(uid,
            ...
            "Для записи на другое время напишите: @kasikovevgenii",
            reply_markup=kb_main())
    except: pass
    await cb.message.edit_text(cb.message.text+"\n\n❌ *Отменено*",
                               parse_mode="Markdown")
...
async def sess_result(cb: types.CallbackQuery):
    parts  = cb.data.split("_")
    result = parts[1]   # yes / no
    stype  = parts[2]   # free / paid
    ds     = parts[3]
    tm     = parts[4]
    appts  = db_get("appts",{})
    ...
    if result == "yes":
        ...
        db_set("appts",appts)
        if stype == "free":
            # Добавляем в очередь ожидания обратной связи
            key = f"{ds}_{tm}"
            pf  = db_get("pending_feedback",{})
            pf[key] = {
                "uid":rec.get("user_id",0) if rec else 0,
                "ds":ds,"time":tm,"dur":dur,
                "name":name,"username":uname,
                "sent":False,
                "remind_at":(datetime.now()+timedelta(hours=0.1)).isoformat()
            }
            db_set("pending_feedback",pf)
            await cb.message.edit_text(
                cb.message.text+"\n\n✅ Отмечено: проведена")
        else:
            # Платная - только статистика + кнопки постоянного клиента
            tl = "💼 Платная"
            kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        db_set("appts",appts)
        stype_label = "Бесплатная" if stype=="free" else "Платная"
        await cb.message.edit_text(
            cb.message.text+f"\n\n❌ {stype_label} не состоялась. Отмечено в статистике.")
...
# ── Постоянный клиент после платной ─────────────────────────────────────
...
async def reg_new(cb: types.CallbackQuery):
    _, _, ds, tm = cb.data.split("_",3)
    set_st(cb.from_user.id, {"step":"adm_reg_from_paid","ref_ds":ds,"ref_tm":tm})
    today = date.today()
    await cb.message.answer("Выберите дату следующей сессии:",
        reply_markup=cal_admin(today.year, today.month))
 
# ════════════════════════════════════════════════════════════
#  АДМИН - КАЛЕНДАРЬ И СЛОТЫ
# ════════════════════════════════════════════════════════════
...
async def aday_open(cb: types.CallbackQuery):
    ...
    # Режим: выбор дня для открытия
    if st.get("step") == "adm_open_day_pick":
        slots = db_get("slots",{})
    ...
    # Режим: выбор дня для открытия недели
    if st.get("step") == "adm_week_pick":
        ...
        await cb.message.answer(
            f"✅ Открыта неделя с *{ds}* по *{end_s}*",
            parse_mode="Markdown", reply_markup=kb_admin())
        return
 
    # Режим: перенос записи - выбор новой даты
    if st.get("step") == "adm_move_new_date":
        st["move_new_ds"] = ds; st["step"] = "adm_move_new_time"
        set_st(uid, st)
        await cb.answer()
        await cb.message.answer(f"Новая дата: *{ds}*\nВведите новое время (например `14:00`):",
                                 parse_mode="Markdown")
        return
 
    # Режим: ручная запись - выбор дня
    if st.get("step") in ("adm_book_date","adm_reg_from_paid"):
        st["book_ds"] = ds; st["step"] = "adm_book_slot_pick"
        set_st(uid, st)
        await cb.answer()
        await cb.message.delete()
        await cb.message.answer(
    ...
    # Прошедший день - добавить сессию
    today = date.today()
    is_past = datetime.strptime(ds,"%Y-%m-%d").date() < today
    if is_past:
        kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def adm_do_open(cb: types.CallbackQuery):
    ...
    await cb.answer(f"✅ День {ds} открыт")
    await cb.message.delete()
    await cb.message.answer(f"✅ День *{ds}* открыт.",
                             parse_mode="Markdown", reply_markup=kb_admin())
...
async def aslot_open(cb: types.CallbackQuery):
    ...
    # Режим: ручная запись - выбор слота
    if st.get("step") == "adm_book_slot_pick":
        st["book_tm"] = tm; st["step"] = "adm_book_plat"
        set_st(uid, st)
        await cb.answer()
        tg = st.get("manual_tg","")
        await cb.message.answer(
    ...
    if rec:
        # Занятый слот - показываем инфо
        tl = "💼 Платная" if rec.get("type")=="paid" else "🆓 Бесплатная"
        conf = "✅ подтверждена" if rec.get("confirmed") else "🟡 не подтверждена"
        await cb.answer(
            f"{rec['name']} | {tl} | {rec.get('duration','—')} | {conf}",
            show_alert=True)
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Подтвердить",
...
async def adm_book_to(cb: types.CallbackQuery):
    ...
    st    = get_st(uid)
    if st.get("manual_tg"):
        st["book_ds"]=ds; st["book_tm"]=tm; st["step"]="adm_book_plat"
        set_st(uid,st)
        await cb.answer()
        await cb.message.answer(
...
async def adm_add_session(cb: types.CallbackQuery):
    ...
    ds  = cb.data[16:]
    uid = cb.from_user.id
    set_st(uid, {"step":"adm_session_name","session_ds":ds})
    await cb.answer()
    await cb.message.answer(f"Добавляю сессию за *{ds}*.\nВведите имя клиента:",
                             parse_mode="Markdown")
 
@dp.callback_query(F.data.startswith("adm_book_new"))
async def adm_book_new(cb: types.CallbackQuery):
    if not is_admin(cb.from_user.username): return
    set_st(cb.from_user.id, {"step":"adm_manual_tg"})
    await cb.answer()
    await cb.message.answer("Введите username клиента в Telegram (без @):")
...
async def adm_book_reg(cb: types.CallbackQuery):
    ...
    regs = db_get("regulars",[])
    if not regs:
        await cb.answer("Нет постоянных клиентов", show_alert=True)
        return
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
...
async def adm_unconf(cb: types.CallbackQuery):
    ...
    if len(rows)==1:
        await cb.message.answer("Неподтверждённых записей нет.", reply_markup=kb_admin())
        return
    await cb.message.answer("🟡 *Неподтверждённые записи:*", parse_mode="Markdown",
                             reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
...
async def adm_reg_del(cb: types.CallbackQuery):
...
async def regday(cb: types.CallbackQuery):
    ...
    st  = get_st(uid); st["reg_day"]=day; st["step"]="adm_reg_time"
    set_st(uid,st)
    ...
    for ds,recs in appts.items():
        ...
        if d.month==now.month and d.year==now.year:
            for r in recs:
                ...
                srcs[r.get("source","бот")]=srcs.get(r.get("source","бот"),0)+1
...
async def adm_open_day_btn(cb: types.CallbackQuery):
    if not is_admin(cb.from_user.username): return
    set_st(cb.from_user.id, {"step":"adm_open_day_pick"})
    today=date.today()
    await cb.message.answer("Нажмите на день для открытия всех слотов:",
        reply_markup=cal_admin(today.year, today.month))
 
@dp.callback_query(F.data == "adm_open_week")
async def adm_open_week_btn(cb: types.CallbackQuery):
    if not is_admin(cb.from_user.username): return
    ...
    for y in [now.year, now.year+1]:
        for m in range(1,13):
            ...
            months_kb.append([InlineKeyboardButton(
                text=f"{MN[m-1]} {y}",
                callback_data=f"adm_bm_{y}_{m:02d}")])
...
async def adm_bm_sel(cb: types.CallbackQuery):
    ...
    month_str = f"{y}-{m:02d}"
    days_n    = calendar.monthrange(y,m)[1]
    appts     = db_get("appts",{})
    conflicts = [f"{y}-{m:02d}-{d:02d}" for d in range(1,days_n+1)
                 if appts.get(f"{y}-{m:02d}-{d:02d}")]
    if conflicts:
        set_st(cb.from_user.id, {"step":"adm_bm_confirm","block_month":month_str})
        ...
        await cb.message.answer(f"⚠️ В {month_str} есть записи:\n{ct}\n\nВсё равно?",
                                 reply_markup=kb)
    else:
        bd = db_get("blocked_dates",[])
...
async def adm_bm_yes(cb: types.CallbackQuery):
    if not is_admin(cb.from_user.username): return
    st = get_st(cb.from_user.id); ms=st.get("block_month","")
    if not ms: return
    ...
    for d in range(1,days_n+1):
        ds=f"{y}-{m:02d}-{d:02d}"
        if ds not in bd: bd.append(ds)
    db_set("blocked_dates",bd); clr_st(cb.from_user.id)
...
async def adm_copy_week(cb: types.CallbackQuery):
    if not is_admin(cb.from_user.username): return
    today   = date.today()
    last_mo = today - timedelta(days=today.weekday()+7)
    this_mo = today - timedelta(days=today.weekday())
    slots   = db_get("slots",{}); count=0
    for i in range(7):
        lds=(last_mo+timedelta(days=i)).strftime("%Y-%m-%d")
        tds=(this_mo+timedelta(days=i)).strftime("%Y-%m-%d")
        d  = this_mo+timedelta(days=i)
        if d >= today:
            slots[tds]=slots[lds].copy() if lds in slots else (ALL_SLOTS.copy() if i<5 else [])
            count+=1
    db_set("slots",slots)
    await cb.answer()
    await cb.message.answer(f"✅ Скопировано/открыто {count} дней", reply_markup=kb_admin())
...
async def handle_text(msg: types.Message):
    ...
    # КРИТИЧНО: команды сбрасывают состояние
    if text.startswith("/"):
        clr_st(uid)
    ...
    if has_bad(text):
        ...
        if cnt >= 5:
            await _auto_block(uid,f"мат ({cnt}x)")
            return
        ...
        if cli_uid:
            try:
                ...
                await bot.send_message(cli_uid,
                    ...
                    "Как обещал - обратная связь и план работы.\n\n"
                    f"{text}")
                await asyncio.sleep(0.5)
                await bot.send_message(cli_uid,
                ...
                await msg.answer("✅ Обратная связь отправлена клиенту.",
                                 reply_markup=kb_admin())
            except Exception as e:
    ...
    if step == "adm_manual_tg":
        tg = text.lstrip("@")
        st["manual_tg"] = tg
        # Если дата и время уже выбраны - к платформе
        if st.get("book_ds") and st.get("book_tm"):
            st["step"]="adm_book_plat"; set_st(uid,st)
    ...
    # ── Ручная запись - имя (если пришёл без username) ──────────────────
    if step == "adm_manual_name":
        st["manual_name"] = text; st["step"]="adm_manual_tg_2"; set_st(uid,st)
        await msg.answer("Введите username клиента в Telegram (без @):")
        return
 
    if step == "adm_manual_tg_2":
        st["manual_tg"]=text.lstrip("@"); st["step"]="adm_book_date"; set_st(uid,st)
        today=date.today()
        await msg.answer("Выберите дату:", reply_markup=cal_admin(today.year,today.month))
        return
 
    # ── Платформа для ручной записи (текстом — не должна приходить, но на всякий случай)
    if step == "adm_book_plat":
        st["platform"]=text; set_st(uid,st)
        await msg.answer("Выберите платформу кнопкой:", reply_markup=kb_platform("adm_back"))
        return
    ...
    if step == "adm_move_new_time":
        ...
        old_tm  = st["move_tm"]; target=st["move_uid"]
        moved   = False
        ...
        clr_st(uid)
        await msg.answer(f"{'✅ Перенесено' if moved else '❌ Не удалось'}",
                         reply_markup=kb_admin())
        return
 
    # ── Добавить сессию за прошедший день ───────────────────────────────
    ...
    if step == "adm_reg_name":
        ...
        await msg.answer("День недели:", reply_markup=kb)
        return
    ...
    if step == "adm_reg_del_name":
        ...
        clr_st(uid)
        await msg.answer(f"✅ @{u} удалён.", reply_markup=kb_admin())
        return
 
    # ── Новый клиент из соцсетей ─────────────────────────────────────────
    if step == "adm_src_username":
        u=text.lstrip("@"); src=st.get("source","other")
        inv=db_get("invited",[]); inv.append({"username":u,"source":src,
            "at":datetime.now().isoformat(),"status":"приглашён"})
        db_set("invited",inv); clr_st(uid)
        try:
            me=await bot.get_me(); link=f"https://t.me/{me.username}"
...
async def stype_cb(cb: types.CallbackQuery):
    if not is_admin(cb.from_user.username): return
    parts=cb.data.split("_"); stype=parts[1]; variant=parts[2]
    uid=cb.from_user.id; st=get_st(uid)
    ds=st.get("session_ds","?"); name=st.get("session_name","?")
    type_map={"free":"🆓 Бесплатная первичная","paid_first":"💼 Платная первичная",
              "paid_repeat":"💼 Платная повторная"}
    tl=type_map.get(f"{stype}_{variant}",stype)
    appts=db_get("appts",{})
    if ds not in appts: appts[ds]=[]
    appts[ds].append({
        "user_id":0,"name":name,"time":"00:00","username":"—",
        "type":stype,"platform":"—","duration":"—","desc":"Ручная сессия",
        "source":"вручную","status":"проведена","confirmed":True,
        "rem_client":True,"rem_admin":True,"session_asked":True,
        "cli_confirmed":True,"followup1":True,"followup3":True
    })
    db_set("appts",appts); clr_st(uid)
    await cb.message.answer(f"✅ Сессия добавлена в статистику.\n{ds} | {tl} | {name}",
                             reply_markup=kb_admin())
...
async def main():
    log.info("="*50)
    log.info("БОТ КАСИКОВА v6.0 ЗАПУЩЕН")
